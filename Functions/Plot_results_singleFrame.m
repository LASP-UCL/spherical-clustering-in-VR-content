function [] = Plot_results_singleFrame(cc_Results,Itersec_ComDect,Itersec_Kmean_1,Itersec_Kmean_2,n_users)
...
for i_frame = 1:n_frames
    ...
    name = sprintf('Frame_%0.2d',i_frame);
    
    %%State-Of-The-Art al.
    total_intersec_sc_1 = Itersec_ComDect.(name); %Community detection results
    total_intersec_sc_2 = Itersec_Kmean_1.(name); % K-means with K from Community detect
    total_intersec_sc_3 = Itersec_Kmean_2.(name); % K-means with K from Clique clustering
    
    
    user_sc_1 = total_intersec_sc_1(2,:);
    user_sc_2 = total_intersec_sc_2(2,:);
    user_sc_3 = total_intersec_sc_3(2,:);
    total_intersec_sc_1 = total_intersec_sc_1(1,:); 
    total_intersec_sc_2 = total_intersec_sc_2(1,:); 
    total_intersec_sc_3 = total_intersec_sc_3(1,:);
...
legend(sprintf(' Clique clustering (%0.2f )',mean(inter_cc_f)*100),sprintf(' Community Detect (%0.2f ) ',mean(inter_sc_1_f)*100),sprintf(' K-means 1 (%0.2f )',mean(inter_sc_3_f)*100),sprintf('K-means 2 (%0.2f )',mean(inter_sc_2_f)*100))
